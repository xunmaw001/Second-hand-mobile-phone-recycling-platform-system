...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">手机回收管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>手机回收列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">手机回收列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									...
									<select name="pinpai" id="pinpaiSelect" class="form-control form-control-sm" aria-controls="tableId">
									</select>
									<label>
										成色
									</label>
									<select name="chengse" id="chengseSelect" class="form-control form-control-sm" aria-controls="tableId">
										<option value=" "> </option>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('neicun')">内存<i id="neicunIcon" class="fa fa-sort"></i></th>

											<th>图片</th>
											<th onclick="sort('riqi')">日期<i id="riqiIcon" class="fa fa-sort"></i></th>

											<th>说明</th>
											<th onclick="sort('gengduo')">更多<i id="gengduoIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			...
			var pinpaiIndex = document.getElementById("pinpaiSelect").selectedIndex;
			var pinpaiOptions = document.getElementById("pinpaiSelect").options;
			var pinpaiSelectedOption = pinpaiOptions[pinpaiIndex]
			if(pinpaiSelectedOption.text != null && pinpaiSelectedOption.text != ''){
				searchForm.pinpai = "%" + pinpaiSelectedOption.text + "%";
			}
			var chengseIndex = document.getElementById("chengseSelect").selectedIndex;
			var chengseOptions = document.getElementById("chengseSelect").options;
			var chengseSelectedOption = chengseOptions[chengseIndex]
			if(chengseSelectedOption.text != null && chengseSelectedOption.text != ''){
				searchForm.chengse = "%" + chengseSelectedOption.text + "%";
			}
			getDataList();
		}		
		// 获取数据列表
        function getDataList() {
			http("shoujihuishou/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
		...
		function setDataRow(item,number){
			...
			var neicunCell  = document.createElement('td');

						 neicunCell.innerHTML = item.neicun;


			row.appendChild(neicunCell);
			...
			var gengduoCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.gengduo && item.gengduo.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.gengduo.indexOf('<img');
				var endIndex = item.gengduo.indexOf('>');				
				gengduoCell.innerHTML = item.gengduo.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.gengduo != null && item.gengduo != "" && item.gengduo.length >= 11){
			  		gengduoCell.innerHTML = item.gengduo.substring(0, 10) + "..."; 
			  	}else{
			  		gengduoCell.innerHTML = item.gengduo;
			 	}
			  }
				  


			row.appendChild(gengduoCell);
            ...
            crossBtn1.innerHTML = "估价"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"shoujigujia\")");
			if(crossBtnControl('估价') == true){
		...
		function sh(){
			...
			httpJson("shoujihuishou/update","POST",data,(res)=>{
				if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("shoujihuishou/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"shoujihuishou");
