...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑手机估价</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>手机估价管理</span></li>
								<li class="breadcrumb-item active"><span>编辑手机估价</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">手机估价信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>检测</label>
										<div>
										<div class="upload">选择文件<input name="file" type="file" class="form-control-file" id="jianceupload"></div>
										<label id="jianceFileName"></label>
										<input name="jiance" id="jiance-input" type="hidden">
										</div>
									</div>
									<div class="form-group col-md-6">
										<label>估价</label>
										<input id="gujia" name="gujia" class="form-control" placeholder="估价">
									</div>
									<div class="form-group col-md-6">
										<label>手机</label>
									...
									<div class="form-group col-md-6">
										...
										<select id="zhuangtaiSelect" name="zhuangtai" class="form-control">
												<option value=""></option>
												<option class="zhuangtaiOption" value="未上门">
													未上门
												</option>
												<option class="zhuangtaiOption" value="已上门">
													已上门
												</option>
												<option class="zhuangtaiOption" value="已取货">
													已取货
												</option>
												<option class="zhuangtaiOption" value="回收完成">
													回收完成
												</option>
										</select>	
									</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "shoujigujia/info/" + id,
		...
		function upload(){
			...
			$('#jianceupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {							
					document.getElementById('jiance-input').setAttribute('value',baseUrl+'file/download?fileName='+data.result.file);
					if(document.getElementById('jianceFileName') != null){
						document.getElementById('jianceFileName').innerHTML = "上传成功!";
					}		
					$("#jianceImg").attr("src",data.result.file);												
				}
			});
		}  
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("shoujigujia/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("shoujigujia/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#jiance-input").val(ruleForm.jiance);
			$("#jianceFileName").html(ruleForm.jiance);
			$("#gujia").val(ruleForm.gujia);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.jiance != null && res.data.jiance != ''){

					$("#jiance-input").val(res.data.jiance);
					$('#jianceFileName').val(res.data.jiance)
					}
					if(res.data != null && res.data != '' && res.data.gujia != null && res.data.gujia != ''){

					$("#gujia").val(res.data.gujia);
					}
